...
*
*   This module contains functions for generating tags for (previously) rust files.
*   May 2019: modified to generate tags for zig files.
*/
		...
		switch (lexer->cur_token)
		{
			...
			case TOKEN_RSHIFT:
				if (angle_level >= 2)
					angle_level -= 2;
				break;
			/* TOKEN_LSHIFT is never interpreted as two <'s in valid Zig code */
			default:
				break;
		}
...
/* Skip tokens until one of the goal tokens is hit. Escapes when level = 0 if there are no goal tokens.
 * Keeps track of balanced <>'s, ()'s, []'s, and {}'s and ignores the goal tokens within those pairings */
static void skipUntil2 (lexerState *lexer, int goal_tokens[], int num_goal_tokens)
{
	int brace_level = 0;
	while (lexer->cur_token != TOKEN_EOF)
	{
		...
		/* Has to be after the token switch to catch the case when we start with the initial level token */
		if (num_goal_tokens == 0 && brace_level == 0)
			break;
		advanceToken(lexer, true);
	}
}
...
static void skipAfterFunctionParameters (lexerState *lexer)
{
	int lookFor2[] = {'}'};
	skipUntil2(lexer,lookFor2,0);
	advanceToken(lexer, true);
}
...
static void parseFn (lexerState *lexer, vString *scope, int parent_kind)
{
	...
	if (valid_signature)
	{
		...
		//skipTypeBlock(lexer);
		skipAfterFunctionParameters(lexer);
		return;
	}
 ...
 * or "var <ident>"
 */
static void parseConstOrVar (lexerState *lexer, vString *scope, int parent_kind)
{
	advanceToken(lexer, true);
	if (lexer->cur_token != TOKEN_IDENT)
		return;

	addTag(lexer->token_str, NULL, NULL, K_VALUE, lexer->line, lexer->pos, scope, parent_kind);
	while(true){
		if(lexer->cur_token == ';' 
		|| lexer->cur_token == TOKEN_EOF){
			advanceToken(lexer,true);
			break;
		}
		advanceToken(lexer,true);
...
static void parseTest(lexerState *lexer, vString *scope, int parent_kind){
	advanceToken(lexer, true);
	
	//printf("Current kind=%d, scope=%s, name=%s\n",parent_kind,scope->buffer,lexer->token_str->buffer);
	addTag(lexer->token_str, NULL, NULL, K_TEST, lexer->line, lexer->pos, scope, parent_kind);
	advanceToken(lexer,true);
	if(lexer->cur_token == '{'){
		int lookFor2[] = {'}'};
		skipUntil2(lexer,lookFor2,0);
...
// const zigConst = struct | enum | (: type) | = 3.2
// zig const statements are complex
static void parseZigConst(lexerState *lexer, vString *scope, int kind){
	...
	vString* ident = vStringNew();
	writeCurTokenToStr(lexer,ident);
	
	vString *name;
	name = vStringNewCopy(lexer->token_str);

	printf("Current kind=%d, scope=%s, name=%s\n",kind,scope->buffer,ident->buffer);
	
	int steps = 0;
	while(steps<10){
		if(lexer->cur_token == ':' || lexer->cur_token == ';')
		{
			addTag(ident, NULL, NULL, K_VALUE, lexer->line, lexer->pos, scope, kind);
			int goalTokens[] = {';'};
			skipUntil2(lexer,goalTokens,0);
			return;
		}else if(strcmp(lexer->token_str->buffer, "struct") == 0){
			addTag(ident, NULL, NULL, K_STRUCT, lexer->line, lexer->pos, scope, kind);
			addToScope(scope, name);
			int lookFor[] = {'{'};
			skipUntil(lexer,lookFor,1);
			parseBlock(lexer, true, K_STRUCT, scope);
			return;
		}else if(strcmp(lexer->token_str->buffer, "enum") == 0){
			addTag(ident, NULL, NULL, K_ENUM, lexer->line, lexer->pos, scope, kind);
			addToScope(scope, name);
			int lookFor[] = {'{'};
			skipUntil(lexer,lookFor,1);
			parseBlock(lexer, true, K_STRUCT, scope);
...
/*
 * Zig is very liberal with nesting, so this function is used pretty much for any block
 */
static void parseBlock (lexerState *lexer, bool delim, int kind, vString *scope)
{
	int level = 1;
	...
	while (lexer->cur_token != TOKEN_EOF)
	{
		if (lexer->cur_token == TOKEN_IDENT)
		{
			...
			else if(strcmp(lexer->token_str->buffer, "const") == 0)
			{
				parseZigConst(lexer, scope, kind);
			}
			else if(strcmp(lexer->token_str->buffer, "var") == 0)
			{
				parseConstOrVar(lexer, scope, kind);
			}
			else if(strcmp(lexer->token_str->buffer, "test") == 0)
			{
				parseTest(lexer, scope, kind);
			}
			else
			{
				advanceToken(lexer, true);
				if (lexer->cur_token == '!')
				{
					skipMacro(lexer);
						...
						addTag(lexer->token_str, NULL, NULL, K_MEMBER, lexer->line, lexer->pos, scope, kind);
						//while(true){
							//if(lexer->cur_token == ',' 
							//|| lexer->cur_token == TOKEN_EOF){
								//advanceToken(lexer,true);
...
extern parserDefinition *ZigParser (void)
{
	static const char *const extensions[] = { "zig", NULL };
	parserDefinition *def = parserNewFull ("Zig", KIND_FILE_ALT);
	def->kindTable = rustKinds;
	def->kindCount = ARRAY_SIZE (rustKinds);
	def->extensions = extensions;
	def->parser = findZigTags;
