...
static void ScanIdentifier(Accessor& styler, Sci_Position& pos, WordList *keywords) {
	...
	if (styler.SafeGetCharAt(pos, '\0') == '!') {
		pos++;
		styler.ColourTo(pos - 1, SCE_ZIG_MACRO);
	} else {
			...
			if (n == '/') {
				...
				if (level == 0) {
					...
					if (state == DocComment || (state == UnknownComment && maybe_doc_comment))
						styler.ColourTo(pos - 1, SCE_ZIG_COMMENTBLOCKDOC);
					else
						styler.ColourTo(pos - 1, SCE_ZIG_COMMENTBLOCK);
					break;
				}
			}
		} else if (c == '/') {
...
static void ResumeRawString(Accessor &styler, Sci_Position& pos, Sci_Position max, int num_hashes, bool ascii_only) {
	...
	styler.ColourTo(pos - 1, ascii_only ? SCE_ZIG_BYTESTRINGR : SCE_ZIG_STRINGR);
}
...
void SCI_METHOD LexerZig::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
	...
	if (initStyle == SCE_ZIG_COMMENTBLOCK || initStyle == SCE_ZIG_COMMENTBLOCKDOC) {
		ResumeBlockComment(styler, pos, max, initStyle == SCE_ZIG_COMMENTBLOCKDOC ? DocComment : NotDocComment, styler.GetLineState(styler.GetLine(pos) - 1));
	} else if (initStyle == SCE_ZIG_COMMENTLINE || initStyle == SCE_ZIG_COMMENTLINEDOC) {
	...
	} else if (initStyle == SCE_ZIG_STRINGR) {
